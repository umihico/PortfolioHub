        ...
        <v-card-text class="text--primary">
            <div class="" style="display: flex;">
                <template v-for="(ratio,lang) in skills">
                    <p class="color-bar" :style="'height: 7px;background-color: '+String((color_dict[lang]) ? color_dict[lang].color : 'red')+'; width: '+ratio+'%'"></p>
                </template>
            </div>
            <template v-for="(ratio,lang) in skills" v-if="hovered">
                <span style="display: inline-block;"><span :style="'background-color: '+String((color_dict[lang]) ? color_dict[lang].color : 'red')+';color: '+String((color_dict[lang]) ? color_dict[lang].color : 'red')+';border-radius: 100px;'">・</span><span class="mr-2 ml-1">{{lang}}</span></span>
            </template>
            <v-row>
                <v-col cols="8">
